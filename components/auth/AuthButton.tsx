      ...
      <div className="absolute inset-0 flex items-center justify-center p-4">
        <div className="bg-slate-900 p-6 rounded-2xl max-w-md w-full border border-yellow-400/20 shadow-2xl shadow-black/50">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-lg font-semibold text-yellow-400">Atención</h2>
          </div>
          ...
          <div className="flex justify-end gap-3 mt-6">
            <button
              onClick={onCancel}
              className="px-4 py-2 text-sm text-white/60 hover:text-white transition rounded-lg hover:bg-white/5 cursor-pointer"
            >
              Cancelar
            </button>
            <button
              onClick={onConfirm}
              className="px-5 py-2 text-sm bg-white text-slate-950 font-semibold rounded-xl hover:bg-white/90 transition cursor-pointer"
            >
...
export function AuthButton() {
  ...
  useEffect(() => {
    if (!data?.user) {
      ...
      fetch("/api/guest-status")
        .then((res) => res.json())
        .then((res) => { setGuestHasData(res.hasData); })
  ...
  if (status === "loading" || checkingGuest) {
    return (
      <button className="rounded-xl border border-white/20 px-4 py-2 text-sm text-white/70 cursor-default" disabled>
        Cargando…
      </button>
    );
  }
  ...
  if (data?.user) {
    return (
      <button
        onClick={() => signOut()}
        className="w-full rounded-xl bg-white text-slate-900 px-4 py-2 text-sm font-medium transition hover:bg-white/90 cursor-pointer text-left"
      >
        Cerrar sesión
      </button>
    );
  ...
  const handleLogin = () => {
    if (guestHasData) setShowModal(true);
    else signIn("google");
  };
